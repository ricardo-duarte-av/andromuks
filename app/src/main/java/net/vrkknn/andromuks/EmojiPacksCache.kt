...
object EmojiPacksCache {
     ...
     * Get all emoji packs
     */
    fun getAll(): List<AppViewModel.EmojiPack> = synchronized(cacheLock) {
        ...
        synchronized(cacheLock) {
            ...
            if (existingIndex >= 0) {
                ...
                if (BuildConfig.DEBUG) {
                    Log.d(
                        TAG,
                        "EmojiPacksCache: updatePack - updated pack ${pack.packName} in room ${pack.roomId}",
                    )
                }
            } else {
                packsList.add(pack)
                if (BuildConfig.DEBUG) {
                    Log.d(
                        TAG,
                        "EmojiPacksCache: updatePack - added new pack ${pack.packName} in room ${pack.roomId}",
                    )
                }
            }
        }
