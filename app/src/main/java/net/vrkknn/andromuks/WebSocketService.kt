...
class WebSocketService : Service() {
    ...
    companion object {
         ...
         */
        fun shouldUseForegroundService(): Boolean = !foregroundStartNotAllowedForThisProcess
         ...
         */
        fun getRecommendedNiceness(): Int = when (getCPUWeight()) {
            CPUWeight.RUSH -> android.os.Process.THREAD_PRIORITY_MORE_FAVORABLE

            // -2
            CPUWeight.EFFICIENT -> android.os.Process.THREAD_PRIORITY_DEFAULT
         ...
         */
        fun isPrimaryInstance(viewModelId: String): Boolean = SyncRepository.getPrimaryViewModelId() == viewModelId

        /**
         * True when a primary id is set and at least one ViewModel is attached in [SyncRepository].
         */
        fun hasPrimaryCallbacks(): Boolean = SyncRepository.getPrimaryViewModelId() != null && SyncRepository.getAttachedViewModels().isNotEmpty()

        /**
         * Debug map for connection / attachment state.
         */
        fun getPrimaryCallbackStatus(): Map<String, Boolean> = mapOf(
        ...
        fun isPrimaryAlive(): Boolean {
            ...
            if (primaryId == null) {
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "STEP 3.1 - Primary health check: No primary ViewModel ID set",
                    )
                }
                return false
            }
            if (!SyncRepository.isPrimaryEntryAlive()) {
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "STEP 3.1 - Primary health check: Primary ViewModel $primaryId missing or not primary in SyncRepository",
                    )
                }
                return false
            }
        ...
        fun registerViewModel(viewModelId: String, isPrimary: Boolean): Boolean {
            ...
            if (ok && BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "STEP 2.1 - registerViewModel → SyncRepository: $viewModelId primary=$isPrimary",
                )
            }
            return ok
        }
        ...
        fun unregisterViewModel(viewModelId: String): Boolean {
            ...
            if (!ok) {
                android.util.Log.w(
                    "WebSocketService",
                    "STEP 2.2 - unregisterViewModel: no attached ViewModel for $viewModelId",
                )
            }
            return ok
        }
         ...
         */
        fun getRegisteredViewModelInfos(): List<ViewModelRegistryInfo> = SyncRepository.getRegisteredViewModelInfos()
        ...
        fun setActivityLogCallback(viewModelId: String, callback: (String, String?) -> Unit): Boolean {
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "setActivityLogCallback: no-op (use SyncRepository)",
                )
            }
            return true
        }
        ...
        fun clearPrimaryCallbacks(viewModelId: String): Boolean {
            ...
            if (pid != viewModelId) {
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "clearPrimaryCallbacks: Instance $viewModelId is not the primary instance ($pid). Nothing to clear.",
                    )
                }
                return false
            }
        ...
        private fun extractReconnectTraceId(text: String): String? = reconnectTraceRegex.find(text)?.groupValues?.getOrNull(1)

        private fun withReconnectTrace(traceId: String?, message: String): String = if (traceId.isNullOrBlank()) message else "[$traceId] $message"
         ...
         */
        private fun invokeReconnectionCallback(
            trigger: ReconnectTrigger,
            lastReceivedId: Int = 0,
            forceColdConnect: Boolean = false,
            logIfMissing: Boolean = true,
        ) {
            val serviceInstance = instance ?: run {
                if (logIfMissing) {
                    android.util.Log.w(
                        "WebSocketService",
                        "Service instance not available - cannot reconnect: ${trigger.toLogString()}",
                    )
                }
                return
            }
            ...
            serviceInstance.serviceScope.launch {
                try {
                    ...
                    if (homeserverUrl.isEmpty() || authToken.isEmpty()) {
                        android.util.Log.w(
                            "WebSocketService",
                            "Cannot reconnect - missing credentials in SharedPreferences",
                        )
                        logActivity(
                            withReconnectTrace(traceId, "Reconnection aborted: missing credentials"),
                    ...
                    if (viewModelToUse == null) {
                        android.util.Log.i(
                            "WebSocketService",
                            "Reconnection with no attached ViewModel — dialing with null AppViewModel (credentials OK)",
                        )
                    }
                    android.util.Log.i(
                        "WebSocketService",
                        "Reconnecting using ViewModel: $viewModelIdStr (primary: $primaryId, attached: ${attached.size})",
                    )
                    logActivity(
                        withReconnectTrace(traceId, "Reconnection dispatch: using ViewModel $viewModelIdStr"),
                        serviceInstance.currentNetworkType.name,
                    )
                    ...
                    if (forceColdConnect) {
                        ...
                        android.util.Log.i(
                            "WebSocketService",
                            "invokeReconnectionCallback: forceColdConnect - no run_id/last_received_event",
                        )
                    } else {
                        ...
                        if (resolvedLastReceivedId != 0 && BuildConfig.DEBUG) {
                            android.util.Log.d(
                                "WebSocketService",
                                "invokeReconnectionCallback: using last_received_id=$resolvedLastReceivedId for connectWebSocket",
                            )
                        }
                    }
                    logActivity(
                        withReconnectTrace(traceId, "Reconnection dispatch: connectWebSocket"),
                        serviceInstance.currentNetworkType.name,
                    )
                    connectWebSocket(
                        homeserverUrl,
                    ...
                    ErrorReportingCoordinator.report(
                        e,
                        "service-initiated reconnection failed: ${trigger.toLogString()}",
                    )
                }
            }
        }
        ...
        private fun invokeOfflineModeCallback(isOffline: Boolean, logIfMissing: Boolean = false) {
            ...
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "Offline mode emitted via SyncRepository: isOffline=$isOffline",
                )
            }
        }
        ...
        fun logPingStatus() {
            ...
            android.util.Log.d(
                "WebSocketService",
                "Pinger status: $status (lastKnownLag: ${serviceInstance.lastKnownLagMs}ms)",
            )
        }
        ...
        fun isForceFreshTimelinePaginatePending(context: Context): Boolean = context.getSharedPreferences("AndromuksAppPrefs", Context.MODE_PRIVATE)
        ...
        fun registerWebSocketSendCallback(callbackId: String, callback: (String, Int, Map<String, Any>) -> Boolean): Boolean {
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "registerWebSocketSendCallback: no-op ($callbackId)",
                )
            }
            return true
        }
        ...
        fun unregisterWebSocketSendCallback(callbackId: String): Boolean {
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "unregisterWebSocketSendCallback: no-op ($callbackId)",
                )
            }
            return true
        }
        ...
        fun registerReceiveCallback(viewModelId: String, viewModel: AppViewModel) {
            ...
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "registerReceiveCallback → SyncRepository.attach ($viewModelId)",
                )
            }
        }
        ...
        fun unregisterReceiveCallback(viewModelId: String): Boolean {
            ...
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "unregisterReceiveCallback → SyncRepository.detach ($viewModelId) ok=$ok",
                )
            }
            return ok
        }
         ...
         */
        @Deprecated(
            "Use registerWebSocketSendCallback instead",
            ReplaceWith("registerWebSocketSendCallback(\"legacy\", callback)"),
        )
        fun setWebSocketSendCallback(callback: (String, Int, Map<String, Any>) -> Boolean) {
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "setWebSocketSendCallback() called (deprecated - use registerWebSocketSendCallback)",
                )
            }
        ...
        fun setPrimaryClearCacheCallback(viewModelId: String, callback: () -> Unit): Boolean {
            ...
            if (pid != null && pid != viewModelId) {
                android.util.Log.w(
                    "WebSocketService",
                    "setPrimaryClearCacheCallback: Another instance ($pid) is already primary. Rejecting $viewModelId",
                )
                return false
            }
            if (pid == null) {
            ...
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "setPrimaryClearCacheCallback: no-op (SyncRepository handles cache events)",
                )
            }
            return true
        }
        ...
        fun setReconnectionCallback(viewModelId: String, callback: (String) -> Unit): Boolean {
            ...
            if (pid != null && pid != viewModelId) {
                android.util.Log.w(
                    "WebSocketService",
                    "setReconnectionCallback: Another instance ($pid) is already primary. Rejecting $viewModelId",
                )
                return false
            }
            if (pid == null) {
            ...
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "setReconnectionCallback: primary id set ($viewModelId); reconnection uses SyncRepository/credentials",
                )
            }
        ...
        private fun processPendingReconnections() {
            ...
            if (!hasReconnectionSignal()) {
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "processPendingReconnections: No credentials / reconnection signal, skipping",
                    )
                }
                return
            }
        ...
        fun setOfflineModeCallback(viewModelId: String, callback: (Boolean) -> Unit): Boolean {
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "setOfflineModeCallback: no-op (offline via SyncRepository)",
                )
            }
            return true
        }
            ...
            serviceInstance.serviceScope.launch {
                try {
                    if (BuildConfig.DEBUG) {
                        android.util.Log.d(
                            "WebSocketService",
                            "triggerBackendHealthCheck called - checking connection state",
                        )
                    }
                    ...
                    if (!serviceInstance.connectionState.isReady() || serviceInstance.webSocket == null) {
                        ...
                        if (BuildConfig.DEBUG) {
                            android.util.Log.d(
                                "WebSocketService",
                                "WebSocket already connected and ready",
                            )
                        }
                    }
                } catch (e: Exception) {
        ...
        fun triggerReconnectionFromExternal(trigger: ReconnectTrigger = ReconnectTrigger.ExternalTriggerNotConnected) {
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "triggerReconnectionFromExternal called: ${trigger.toLogString()}",
                )
            }
        ...
        fun triggerReconnectionSafely(trigger: ReconnectTrigger) {
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "triggerReconnectionSafely called: ${trigger.toLogString()}",
                )
            }
            ...
            if (!hasReconnectionSignal()) {
                android.util.Log.w(
                    "WebSocketService",
                    "Reconnection callback not set (AppViewModel not available) - cannot reconnect. Waiting for app to connect WebSocket properly.",
                )
            ...
            if (requestId == serviceInstance.lastPingRequestId) {
                ...
                if (serviceInstance.pingInFlight) {
                    ...
                    if (serviceInstance.consecutivePingTimeouts > 0) {
                        android.util.Log.i(
                            "WebSocketService",
                            "Pong received - resetting consecutive failures (was: ${serviceInstance.consecutivePingTimeouts})",
                        )
                        logActivity(
                            "Pong Received - Recovered (was ${serviceInstance.consecutivePingTimeouts} failures)",
                            serviceInstance.currentNetworkType.name,
                        )
                    }
                    serviceInstance.consecutivePingTimeouts = 0
                ...
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "Pong received for non-matching requestId: $requestId (expected: ${serviceInstance.lastPingRequestId}) - connection is alive but pong is stale",
                    )
                }
                ...
                if (serviceInstance.consecutivePingTimeouts > 0) {
                    android.util.Log.i(
                        "WebSocketService",
                        "Stale pong received - connection is alive, resetting consecutive failures (was: ${serviceInstance.consecutivePingTimeouts})",
                    )
                    serviceInstance.consecutivePingTimeouts = 0
                }
            }
        ...
        fun triggerReconnection(trigger: ReconnectTrigger) {
            ...
            if (!serviceInstance.connectionState.isReconnectingPhase()) {
                ...
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "Already reconnecting, ignoring trigger: ${trigger.toLogString()}",
                    )
                }
            }
        }
        ...
        fun setWebSocket(webSocket: WebSocket) {
            ...
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "Current connection state: ${serviceInstance.connectionState}",
                )
            }
            ...
            if (serviceInstance.connectionState.isConnecting() && serviceInstance.webSocket != null) {
                android.util.Log.w(
                    "WebSocketService",
                    "WebSocket connection already in progress with active socket, ignoring new connection",
                )
                return
            }
            ...
            if (serviceInstance.connectionState.isReady() && serviceInstance.webSocket != null) {
                android.util.Log.w(
                    "WebSocketService",
                    "WebSocket already Ready with live socket - rejecting replacement attempt (likely stale reconnection job)",
                )
            ...
            if (serviceInstance.connectionState.isDialOrSyncing()) {
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "Replacing existing WebSocket connection (reconnection)",
                    )
                }
        ...
        fun clearWebSocket(reason: String = "Unknown", closeCode: Int? = null, closeReason: String? = null) {
            ...
            if (closeCode != null) {
                ...
                android.util.Log.i(
                    "WebSocketService",
                    "WebSocket closed with code $closeCode: ${closeReason ?: reason}",
                )
            }
            ...
            if (!needsClearing) {
                ...
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "clearWebSocket() called but already disconnected - skipping",
                    )
                }
                return
            }
            ...
            android.util.Log.w(
                "WebSocketService",
                "clearWebSocket() called - setting connection state to DISCONNECTED (was: ${serviceInstance.connectionState})",
            )
         ...
         */
        fun getLastCloseCode(): Int? = instance?.lastCloseCode

        /**
         * PHASE 4.1: Get last WebSocket close reason
         */
        fun getLastCloseReason(): String? = instance?.lastCloseReason
        ...
        fun isWebSocketConnected(): Boolean {
            ...
            if (BuildConfig.DEBUG && !isConnected && serviceInstance.connectionState.isReady()) {
                ...
                android.util.Log.w(
                    "WebSocketService",
                    "isWebSocketConnected(): State is READY but webSocket is null - state corruption detected",
                )
            }
            return isConnected
        }
         ...
         */
        fun getConnectionState(): ConnectionState? = instance?.connectionState
         ...
         */
        fun lastMessageAtMs(): Long = instance?.lastMessageReceivedTimestamp ?: 0L
         ...
         */
        fun getCurrentNetworkType(): NetworkType = instance?.currentNetworkType ?: NetworkType.NONE
        ...
        fun detectAndRecoverStateCorruption(): Boolean {
            ...
            if (hasWebSocket && !isReady && !serviceInstance.connectionState.isDialOrSyncing()) {
                android.util.Log.w(
                    "WebSocketService",
                    "CORRUPTION: WebSocket exists but state is not active - recovering",
                )
            ...
            if (serviceInstance.pongTimeoutJob?.isActive == true &&
                ...
                android.util.Log.w(
                    "WebSocketService",
                    "CORRUPTION: Pong timeout running but not ready - stopping timeout",
                )
                serviceInstance.pongTimeoutJob?.cancel()
                serviceInstance.pongTimeoutJob = null
                corruptionDetected = true
         ...
         */
        fun getWebSocket(): WebSocket? = instance?.webSocket
        ...
        fun sendCommand(command: String, requestId: Int, data: Map<String, Any>): Boolean {
            ...
            if (ws == null) {
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "sendCommand() called but WebSocket is null: $command",
                    )
                }
                return false
            }
            ...
            if (!serviceInstance.connectionState.isReady()) {
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "sendCommand() called but not ready (state: ${serviceInstance.connectionState}): $command",
                    )
                }
                return false
            }
        ...
        fun getCurrentRunId(): String {
            val serviceInstance = instance ?: return ""
            return try {
                val prefs = serviceInstance.getSharedPreferences(
                    "AndromuksAppPrefs",
         ...
         */
        fun getLastConnectionUrl(context: Context): String = try {
            context.getSharedPreferences("AndromuksAppPrefs", Context.MODE_PRIVATE)
            ...
            while (System.currentTimeMillis() - startTime < timeoutMs) {
                if (instance != null) {
                    if (BuildConfig.DEBUG) {
                        android.util.Log.d(
                            "WebSocketService",
                            "Service instance ready after ${System.currentTimeMillis() - startTime}ms",
                        )
                    }
                    return instance
                }
        ...
        fun connectWebSocket(
            ...
            getServiceScope().launch {
                try {
                    ...
                    android.util.Log.i(
                        "WebSocketService",
                        "connectWebSocket() called - reason: $reason, isReconnection: $isReconnection",
                    )
                    ...
                    if (!validated) {
                        val disconnectedMs = if (serviceInstance.connectionLostAt >
                            0
                        ) {
                            System.currentTimeMillis() - serviceInstance.connectionLostAt
                        } else {
                            0L
                        }
                        ...
                        if (serviceInstance.connectionLostAt > 0 && disconnectedMs > offlineThresholdMs) {
                            android.util.Log.i(
                                "WebSocketService",
                                "connectWebSocket: Validation timeout but disconnected >60 min (${disconnectedMs}ms) - connecting cold (no run_id/last_received_event)",
                            )
                            logActivity(
                                withReconnectTrace(traceId, "Validation Timeout - Connecting Cold (>60 min offline)"),
                                serviceInstance.currentNetworkType.name,
                            )
                            ...
                            android.util.Log.w(
                                "WebSocketService",
                                "connectWebSocket: Network not validated within ${NETWORK_VALIDATION_TIMEOUT_MS}ms - aborting (will retry on next trigger)",
                            )
                            serviceInstance.showWebSocketToast("Network not validated")
                            logActivity(
                                withReconnectTrace(traceId, "connectWebSocket aborted: network not validated"),
                                serviceInstance.currentNetworkType.name,
                            )
                            updateConnectionState(ConnectionState.Disconnected)
                            return@launch
                    ...
                    if (effectiveLastReceivedId != 0) {
                        ...
                        if (BuildConfig.DEBUG) {
                            android.util.Log.d(
                                "WebSocketService",
                                "Stored last_received_id: $effectiveLastReceivedId for reconnection",
                            )
                        }
                    }
                    ...
                    if (BuildConfig.DEBUG) {
                        android.util.Log.d(
                            "WebSocketService",
                            "Connecting WebSocket (isReconnection=$effectiveIsReconnection)",
                        )
                    }
                    logActivity(
                        withReconnectTrace(traceId, "WebSocket dial started (isReconnection=$effectiveIsReconnection)"),
                        serviceInstance.currentNetworkType.name,
                    )
                    ...
                    net.vrkknn.andromuks.utils.connectToWebsocket(
                        ...
                        client,
                        token,
                        serviceInstance.applicationContext,
                        appViewModel,
                        reason = reason,
                    ...
                    if (BuildConfig.DEBUG) {
                        android.util.Log.d(
                            "WebSocketService",
                            "NetworkUtils.connectToWebsocket() call completed",
                        )
                    }
                } catch (e: Exception) {
        ...
        fun updateInitializingProgress(pendingCount: Int = 0, processedCount: Int = 0) {
            ...
            if (currentState is ConnectionState.Initializing) {
                ...
                if (newPending > 0 && newProcessed >= newPending) {
                    ...
                    android.util.Log.i(
                        "WebSocketService",
                        "All sync_completes processed - state transition: INITIALIZING → READY",
                    )
                ...
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "updateInitializingProgress called but state is $currentState (expected INITIALIZING)",
                    )
                }
            }
        }
        ...
        fun updateLastSyncTimestamp() {
            ...
            updateConnectionStatus(
                serviceInstance.connectionState.isReady(),
                serviceInstance.lastKnownLagMs,
                serviceInstance.lastSyncTimestamp,
            )
        }
         ...
         */
        fun isReconnectingWithLastReceivedEvent(): Boolean = instance?.isReconnectingWithLastReceivedEvent ?: false
        ...
        fun startPingLoopOnInitComplete() {
            ...
            if (!serviceInstance.connectionState.isInitializing() && !serviceInstance.connectionState.isReady()) {
                android.util.Log.w(
                    "WebSocketService",
                    "Init_complete received but connection state is ${serviceInstance.connectionState} - not starting ping loop",
                )
                return
            }
            ...
            if (currentState is ConnectionState.Initializing && currentState.pendingSyncCount == 0) {
                android.util.Log.i(
                    "WebSocketService",
                    "Init_complete received with no pending sync_completes - transitioning to READY immediately",
                )
                updateConnectionState(ConnectionState.Ready)
        ...
        fun clearReconnectionState() {
            ...
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "clearReconnectionState called (no-op - we no longer track last_received_id)",
                )
            }
        }
        ...
        fun resetReconnectionState() {
            ...
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "Reset reconnection state (reconnection job cancelled, retry count reset)",
                )
            }
        }
        ...
        fun onInitCompleteReceived(source: String = "init_complete") {
            ...
            if (isDuplicateWithinWindow) {
                logActivity(
                    withReconnectTrace(traceId, "Init Complete Duplicate ($source)"),
                    serviceInstance.currentNetworkType.name,
                )
            } else {
                logActivity(
                    withReconnectTrace(traceId, "Init Complete Received ($source)"),
                    serviceInstance.currentNetworkType.name,
                )
            }
            updateConnectionStatus(true, null, serviceInstance.lastSyncTimestamp)
        ...
        fun scheduleReconnection(trigger: ReconnectTrigger) {
            ...
            if (serviceInstance.currentNetworkType == NetworkType.NONE) {
                ...
                android.util.Log.w(
                    "WebSocketService",
                    "No network — state=WaitingForNetwork(lastEventId=$lastEv), queued trigger=$trigger",
                )
            ...
            synchronized(serviceInstance.reconnectionLock) {
                ...
                if (serviceInstance.connectionState.isReconnectingPhase()) {
                    ...
                    if (timeSinceReconnect > 30_000) {
                        ...
                        android.util.Log.w(
                            "WebSocketService",
                            "Reconnection stuck for ${timeSinceReconnect}ms - resetting to allow new reconnection attempt: $reasonLabel",
                        )
                        serviceInstance.reconnectionJob?.cancel()
                        serviceInstance.reconnectionJob = null
                        ...
                        if (trigger.interruptsSlowReconnection()) {
                            android.util.Log.w(
                                "WebSocketService",
                                "Reconnection in progress for ${timeSinceReconnect}ms but network changed - interrupting: $reasonLabel",
                            )
                            serviceInstance.reconnectionJob?.cancel()
                            serviceInstance.reconnectionJob = null
                            // Fall through to start new reconnection
                            ...
                            if (BuildConfig.DEBUG) {
                                android.util.Log.d(
                                    "WebSocketService",
                                    "Already reconnecting (${timeSinceReconnect}ms), dropping redundant request: $reasonLabel",
                                )
                            }
                            logActivity(
                ...
                if (currentTime - serviceInstance.lastReconnectionTime < MIN_RECONNECTION_INTERVAL_MS) {
                    if (BuildConfig.DEBUG) {
                        android.util.Log.d(
                            "WebSocketService",
                            "Too soon since last reconnection, ignoring: $reasonLabel",
                        )
                    }
                    logActivity(
                ...
                if (lastCloseCode != null) {
                    android.util.Log.i(
                        "WebSocketService",
                        "Last WebSocket close code: $lastCloseCode (${serviceInstance.lastCloseReason})",
                    )
                }
                ...
                serviceInstance.reconnectionJob = serviceInstance.serviceScope.launch {
                    ...
                    try {
                        ...
                        if (serviceInstance.currentNetworkType == NetworkType.NONE) {
                            android.util.Log.w(
                                "WebSocketService",
                                "Reconnection job: Network is NONE - waiting for network to return",
                            )
                            serviceInstance.showWebSocketToast("No network - waiting")
                            ...
                            android.util.Log.i(
                                "WebSocketService",
                                "Reconnection job: Network returned - continuing reconnection",
                            )
                        }

                        // STATE A: First step - wait for NET_CAPABILITY_VALIDATED. Do not connect until validated.
                        logActivity(
                            "[$reconnectTraceId] Reconnection waiting for validated network",
                            serviceInstance.currentNetworkType.name,
                        )
                        ...
                        if (!networkValidated) {
                            ...
                            if (serviceInstance.connectionLostAt > 0 && disconnectedMs > 60_000L) {
                                android.util.Log.i(
                                    "WebSocketService",
                                    "Reconnection job: Validation timeout but disconnected >1 min (${disconnectedMs}ms) - connecting cold",
                                )
                                logActivity(
                                    "[$reconnectTraceId] Validation Timeout - Connecting Cold (>1 min offline)",
                                    serviceInstance.currentNetworkType.name,
                                )
                            ...
                            android.util.Log.w(
                                "WebSocketService",
                                "Reconnection job: Network not validated within ${NETWORK_VALIDATION_TIMEOUT_MS}ms - aborting (will retry on next trigger)",
                            )
                            logActivity(
                                "[$reconnectTraceId] Network Not Validated - Will Retry",
                                serviceInstance.currentNetworkType.name,
                            )
                            ...
                            serviceInstance.serviceScope.launch {
                                ...
                                if (stillDisconnected && !isReconnectingOrConnecting()) {
                                    logActivity(
                                        "[$reconnectTraceId] Reconnection Retry: post-validation-timeout backoff",
                                        serviceInstance.currentNetworkType.name,
                                    )
                        ...
                        if (BuildConfig.DEBUG) {
                            android.util.Log.d(
                                "WebSocketService",
                                "Reconnection job: Network validated - proceeding",
                            )
                        }
        ...
        fun restartWebSocket(trigger: ReconnectTrigger = ReconnectTrigger.Unclassified("Unknown reason")) {
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "Restarting WebSocket connection - trigger: $trigger",
                )
            }
            ...
            if (currentWebSocket != null) {
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "Closing existing WebSocket before restart",
                    )
                }
                try {
            ...
            if (trigger !is ReconnectTrigger.NetworkAvailable) {
                ...
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "Network restored - skipping clearWebSocket to preserve state",
                    )
                }
            }
            ...
            serviceInstance.serviceScope.launch {
                ...
                if (trigger is ReconnectTrigger.FcmPush) {
                    android.util.Log.w(
                        "WebSocketService",
                        "FCM push trigger — cannot connect WebSocket without AppViewModel",
                    )
                    android.util.Log.w(
                        "WebSocketService",
                        "WebSocket will remain in 'Connecting...' state until app is opened manually",
                    )
                    return@launch
                }
                ...
                if (trigger is ReconnectTrigger.ServiceRestarted) {
                    android.util.Log.w(
                        "WebSocketService",
                        "restartWebSocket(ServiceRestarted) — would loop; AppViewModel should connect directly",
                    )
                    return@launch
                }
    ...
    private var lastMessageReceivedTimestamp = 0L
                ...
                try {
                    ...
                    if (stateCorruptionCheckCounter >= 30) {
                        if (BuildConfig.DEBUG) {
                            android.util.Log.d(
                                "WebSocketService",
                                "Unified monitoring: state corruption + primary health checks",
                            )
                        }
                        ...
                        if (!isAlive) {
                            android.util.Log.w(
                                "WebSocketService",
                                "STEP 3.2 - Primary health check: Primary ViewModel is NOT alive (primaryId=$primaryId)",
                            )
                            val registeredCount = getRegisteredViewModelIds().size
                            val callbackStatus = getPrimaryCallbackStatus()
                            android.util.Log.w(
                                "WebSocketService",
                                "STEP 3.2 - Health check details: registeredViewModels=$registeredCount, callbackStatus=$callbackStatus",
                            )
                            if (primaryId != null) {
                                android.util.Log.i(
                                    "WebSocketService",
                                    "STEP 3.3 - Primary ViewModel $primaryId detected as dead - attempting automatic promotion",
                                )
                                ...
                                if (registeredCount > 0) {
                                    android.util.Log.i(
                                        "WebSocketService",
                                        "STEP 3.3 - No primary set but $registeredCount ViewModels available - attempting promotion",
                                    )
                                    SyncRepository.promoteNextPrimary("no_primary_set")
                                } else {
                                    android.util.Log.w(
                                        "WebSocketService",
                                        "STEP 3.3 - No primary and no ViewModels available - next MainActivity launch will become primary",
                                    )
                                }
                            }
                        }
                    ...
                    val isStuckConnectingNoTimeouts = if (currentState.isConnecting()) {
                        ...
                        val hasAnyActiveTimeout =
                            hasActiveRunIdTimeout || hasActiveInitCompleteTimeout || hasActiveHardTimeout
                    ...
                    val isStuckReconnecting = when {
                        currentState.isReconnectingPhase() -> {
                            val timeSinceReconnect = if (lastReconnectionTime >
                                0
                            ) {
                                currentTime - lastReconnectionTime
                            } else {
                                0
                            }
                    ...
                    if (isStuckConnecting || isStuckConnectingSimple || isStuckConnectingNoTimeouts || isStuckReconnecting) {
                        ...
                        android.util.Log.w(
                            "WebSocketService",
                            "Unified monitoring: Detected stuck state ($currentState) - $stuckReason - forcing recovery",
                        )
                    ...
                    if (currentState.isDisconnected() &&
                        ...
                        android.util.Log.w(
                            "WebSocketService",
                            "Health check: Stuck in DISCONNECTED with no active reconnection — scheduling recovery",
                        )
                        logActivity("Health Check - Stuck DISCONNECTED (no reconnection job)", currentNetworkType.name)
                        scheduleReconnection(
                            ReconnectTrigger.HealthCheckRecovery(
                                "Stuck in DISCONNECTED with no active reconnection job",
                            ),
                        )
                    ...
                    if (timeSinceNotificationUpdate > 60_000 && !currentState.isReady()) {
                        android.util.Log.w(
                            "WebSocketService",
                            "Unified monitoring: Notification stale (${timeSinceNotificationUpdate}ms old) - forcing update",
                        )
                        updateConnectionStatus(
    ...
    private fun validateCallbacks() {
        ...
        if (!isConnected && !hasSignal) {
            android.util.Log.w(
                "WebSocketService",
                "Callback health check: no credentials / reconnection signal and connection is not READY",
            )
        ...
        networkMonitor = NetworkMonitor(
            ...
            onNetworkAvailable = { networkType ->
                ...
                networkChangeDebounceJob = serviceScope.launch {
                    ...
                    android.util.Log.i(
                        "WebSocketService",
                        "Network available: $networkType - waiting for validation before reconnecting",
                    )
                    ...
                    if (!networkValidated) {
                        ...
                        android.util.Log.w(
                            "WebSocketService",
                            "Network available but not validated within ${WebSocketService.NETWORK_VALIDATION_TIMEOUT_MS}ms - not reconnecting (will retry on next event)",
                        )
                    ...
                    if (connectionState.isDisconnected()) {
                        ...
                        if (alreadyConnecting) {
                            if (BuildConfig.DEBUG) {
                                android.util.Log.d(
                                    "WebSocketService",
                                    "Network validated but connection already in progress - skipping duplicate reconnection",
                                )
                            }
                            return@launch
                        }
                        ...
                        android.util.Log.w(
                            "WebSocketService",
                            "Network validated but WebSocket disconnected - triggering reconnection",
                        )
                        ...
                        if (isStuckNoTimeouts || isStuckTooLong) {
                            ...
                            if (BuildConfig.DEBUG) {
                                android.util.Log.d(
                                    "WebSocketService",
                                    "Network validated but WebSocket already connecting (${timeSinceConnect}ms, timeouts active: $hasAnyActiveTimeout) - waiting for init_complete",
                                )
                            }
                        }
                        ...
                        if (!hasActiveJob) {
                            ...
                            android.util.Log.w(
                                "WebSocketService",
                                "Network available while in RECONNECTING state with no active job - reconnecting immediately",
                            )
                            ...
                            updateConnectionState(ConnectionState.Disconnected)
                            invokeReconnectionCallback(
                                ReconnectTrigger.NetworkValidated(newNetworkType),
                                lastReceivedId,
                            )
                            return@launch
                        }
                        ...
                        android.util.Log.w(
                            "WebSocketService",
                            "Network validated while in RECONNECTING state - checking backend health before reconnecting",
                        )
                        logActivity("Network Validated - Reconnecting", currentNetworkType.name)
                        ...
                        android.util.Log.w(
                            "WebSocketService",
                            "Network type changed while connected - forcing reconnection ($previousNetworkType → $newNetworkType)",
                        )
                        ...
                        if (BuildConfig.DEBUG) {
                            android.util.Log.d(
                                "WebSocketService",
                                "Network available - same network type and identity - no action needed (ping/pong will handle failures)",
                            )
                        }
                    }
                }
            ...
            onNetworkTypeChanged = { previousType, newType ->
                ...
                android.util.Log.w(
                    "WebSocketService",
                    "Network type changed - forcing reconnection ($previousNetworkType → $newNetworkType)",
                )
                ...
                if (connectionState.isConnecting()) {
                    android.util.Log.w(
                        "WebSocketService",
                        "Network type changed while in CONNECTING state - clearing connection before reconnecting",
                    )
            ...
            onNetworkIdentityChanged = { previousType, previousIdentity, newType, newIdentity ->
                ...
                android.util.Log.i(
                    "WebSocketService",
                    "Network identity changed: $previousType ($previousIdentity) → $newType ($newIdentity) - ignoring (WiFi name irrelevant)",
                )
                logActivity(
                    "Network Identity Changed: $previousType ($previousIdentity) → $newType ($newIdentity)",
                    newType.name,
                )
     ...
     */
    private fun convertNetworkType(networkType: NetworkMonitor.NetworkType): NetworkType = when (networkType) {
    ...
    private suspend fun waitForNetworkValidation(timeoutMs: Long = 5000L): Boolean {
        ...
        if (BuildConfig.DEBUG) {
            android.util.Log.d(
                "WebSocketService",
                "Network validation: Waiting up to ${timeoutMs}ms for validation",
            )
        }
        ...
        while (System.currentTimeMillis() - startTime < timeoutMs) {
            ...
            if (currentNetwork != null && currentNetwork != activeNetwork) {
                ...
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "Network validation: Network switched during validation - re-checking on new network",
                    )
                }
        ...
        android.util.Log.w(
            "WebSocketService",
            "Network validation: Timeout after ${timeoutMs}ms - network may be slow or captive portal",
        )
    ...
    private suspend fun tryFallbackNetworkValidation(): Boolean {
        ...
        try {
            android.util.Log.i(
                "WebSocketService",
                "Fallback validation: Checking backend health with ${fallbackBackoffDelayMs}ms delay",
            )
            ...
            val isHealthy = try {
                ...
                healthClient.newCall(request).execute().use { response ->
                    ...
                    android.util.Log.i(
                        "WebSocketService",
                        "Fallback validation: Backend health check: HTTP ${response.code} (healthy=$healthy)",
                    )
                    healthy
                }
            } catch (e: Exception) {
                android.util.Log.w(
                    "WebSocketService",
                    "Fallback validation: Backend health check failed: ${e.message}",
                    e,
                )
                false
            }
            ...
            if (isHealthy) {
                ...
                android.util.Log.i(
                    "WebSocketService",
                    "Fallback validation: Backend reachable - proceeding with reconnection",
                )
                return true
            } else {
                ...
                android.util.Log.w(
                    "WebSocketService",
                    "Fallback validation: Backend not reachable - next backoff: ${fallbackBackoffDelayMs}ms",
                )
                return false
            }
        } catch (e: Exception) {
    ...
    private fun shouldReconnectOnNetworkChange(previousType: NetworkType, newType: NetworkType): Boolean {
        ...
        if (previousType == NetworkType.NONE && newType != NetworkType.NONE) {
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "shouldReconnectOnNetworkChange: Offline → Online - reconnecting",
                )
            }
            return true
        }
        ...
        if (previousType != NetworkType.NONE && newType == NetworkType.NONE) {
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "shouldReconnectOnNetworkChange: Online → Offline - not reconnecting",
                )
            }
            return false
        }
        ...
        if (previousType == NetworkType.CELLULAR && newType == NetworkType.WIFI) {
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "shouldReconnectOnNetworkChange: Mobile → WiFi - reconnecting for better network",
                )
            }
            return true
        }
        ...
        if (previousType == NetworkType.WIFI && newType == NetworkType.CELLULAR) {
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "shouldReconnectOnNetworkChange: WiFi → Mobile - not reconnecting (network still functional)",
                )
            }
            return false
        }
        ...
        if (previousType == newType && connectionState.isReady()) {
            ...
            if (isHealthy) {
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "shouldReconnectOnNetworkChange: Same network type ($previousType) and connection healthy - not reconnecting",
                    )
                }
                return false
            } else {
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "shouldReconnectOnNetworkChange: Same network type but connection unhealthy (sync: ${timeSinceSync}ms, lag: $lastKnownLagMs) - reconnecting",
                    )
                }
                return true
            }
        ...
        if ((previousType == NetworkType.WIFI && newType == NetworkType.WIFI) ||
            ...
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "shouldReconnectOnNetworkChange: Same type but different network ($previousType → $newType) - reconnecting",
                )
            }
            return true
        }
        ...
        if (previousType != newType) {
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "shouldReconnectOnNetworkChange: Other transition ($previousType → $newType) - reconnecting",
                )
            }
            return true
        }
        ...
        if (BuildConfig.DEBUG) {
            android.util.Log.d(
                "WebSocketService",
                "shouldReconnectOnNetworkChange: No network type change - not reconnecting",
            )
        }
        return false
    }
    ...
    private fun startPongTimeout(pingRequestId: Int) {
        ...
        pongTimeoutJob = serviceScope.launch {
            ...
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "Pong clear timeout - clearing pingInFlight for next ping",
                )
            }
            pingInFlight = false
        }
    ...
    private fun startRunIdTimeout() {
                ...
                try {
                    ...
                    if (runIdReceived) {
                        if (BuildConfig.DEBUG) {
                            android.util.Log.d(
                                "WebSocketService",
                                "Run ID timeout expired but already received - ignoring",
                            )
                        }
                        return@launch
                    }
                    ...
                    if (!connectionState.isDialOrSyncing()) {
                        if (BuildConfig.DEBUG) {
                            android.util.Log.d(
                                "WebSocketService",
                                "Run ID timeout expired but connection not active - ignoring",
                            )
                        }
                        return@launch
                    }
                    ...
                    if (BuildConfig.DEBUG) {
                        android.util.Log.d(
                            "WebSocketService",
                            "Run ID timeout job cancelled (run_id received)",
                        )
                    }
                    throw e
                } catch (e: Exception) {
        ...
        if (runIdTimeoutJob == null || !runIdTimeoutJob!!.isActive) {
            android.util.Log.w(
                "WebSocketService",
                "Run ID timeout job failed to schedule - unified monitoring will detect stuck state",
            )
        }
    }
    ...
    private fun startInitCompleteTimeout() {
        ...
        if (BuildConfig.DEBUG) {
            android.util.Log.d(
                "WebSocketService",
                "Starting init_complete timeout: ${baseTimeoutMs}ms base (will extend by ${INIT_COMPLETE_EXTENSION_PER_MESSAGE_MS}ms per message)",
            )
        }
    ...
    private fun extendInitCompleteTimeoutOnMessage() {
        ...
        if (BuildConfig.DEBUG) {
            ...
            android.util.Log.d(
                "WebSocketService",
                "Extended init_complete timeout on message #$messagesReceivedWhileWaitingForInitComplete - new timeout: ${remainingMs}ms from now",
            )
        }
    ...
    private fun restartInitCompleteTimeoutWithNewEndTime(endTime: Long) {
        ...
        if (remainingMs <= 0) {
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "Not restarting init_complete timeout - already expired",
                )
            }
            return
        }
                ...
                try {
                    ...
                    if (!connectionState.isDialOrSyncing()) {
                        if (BuildConfig.DEBUG) {
                            android.util.Log.d(
                                "WebSocketService",
                                "Init complete timeout expired but not in dial/sync phase - ignoring",
                            )
                        }
                        return@launch
                    }
    ...
    private suspend fun handleInitCompleteTimeout() {
        ...
        if (alreadyReconnecting) {
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "Init complete timeout but already reconnecting - skipping duplicate notification and retry",
                )
            }
                ...
                try {
                    ...
                    if (!connectionState.isConnecting()) {
                        if (BuildConfig.DEBUG) {
                            android.util.Log.d(
                                "WebSocketService",
                                "Hard connecting timeout expired but no longer in Connecting state - ignoring",
                            )
                        }
                        return@launch
                    }
                    ...
                    if (BuildConfig.DEBUG) {
                        android.util.Log.d(
                            "WebSocketService",
                            "Hard connecting timeout job cancelled (state transitioned)",
                        )
                    }
                    throw e
                } catch (e: Exception) {
    ...
    override fun onCreate() {
        ...
        if (persistedRequestId != 0) {
            ...
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "Restored last_received_request_id from SharedPreferences: $persistedRequestId",
                )
            }
        }
        ...
        if (BuildConfig.DEBUG) {
            android.util.Log.d(
                "WebSocketService",
                "Service startup - run_id will be read from SharedPreferences when needed",
            )
        }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (!hasCallback) {
            android.util.Log.w(
                "WebSocketService",
                "Service started without login credentials - notification will show 'Waiting for app...'",
            )
        }
        ...
        if (isBatterySaverUserDisconnected(this)) {
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "onStartCommand: batterySaver user-disconnected — returning START_NOT_STICKY",
                )
            }
            return START_NOT_STICKY
        }
        ...
        try {
            ...
            if (isBatterySaverUserDisconnected(applicationContext)) {
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "onDestroy: batterySaver user-disconnected — skipping auto-restart",
                    )
                }
            } else {
    ...
    override fun onTaskRemoved(rootIntent: Intent?) {
        ...
        android.util.Log.w(
            "WebSocketService",
            "onTaskRemoved() called - app removed from recent apps, scheduling AlarmManager restart",
        )
        ...
        val base = try {
            ...
            if (homeserverUrl.isNotBlank()) {
                ...
                val url = java.net.URL(
                    if (homeserverUrl.startsWith("http")) homeserverUrl else "https://$homeserverUrl",
                )
                url.host
            } else {
                "Andromuks"
     ...
     */
    private fun getNetworkTypeDisplayName(networkType: NetworkType): String = when (networkType) {
    ...
    private fun getConnectionHealthIndicator(lagMs: Long?, lastSyncTimestamp: Long?): String {
        ...
        return when {
            ...
            lagMs < 200 && timeSinceSync < 30_000 -> "🟢"

            // Green
            // Good: reasonable lag and recent sync
            lagMs < 500 && timeSinceSync < 60_000 -> "🟡"

            // Yellow
            // Poor: high lag or old sync
            lagMs >= 1000 || timeSinceSync >= 120_000 -> "🔴"
    ...
    private fun checkBatteryOptimizationStatus() {
        try {
            ...
            if (!isIgnoringBatteryOptimizations) {
                android.util.Log.w(
                    "WebSocketService",
                    "Battery optimization is ENABLED for this app - service may be killed by system",
                )
                android.util.Log.w(
                    "WebSocketService",
                    "To prevent service from being killed, disable battery optimization in Settings > Apps > Andromuks > Battery",
                )
                android.util.Log.w(
                    "WebSocketService",
                    "Or use the in-app permissions screen to request battery optimization exemption",
                )
            } else {
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "Battery optimization is disabled - service should remain stable",
                    )
                }
            }
        } catch (e: Exception) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                ...
                if (alarmManager.canScheduleExactAlarms()) {
                    ...
                    if (BuildConfig.DEBUG) {
                        android.util.Log.d(
                            "WebSocketService",
                            "Scheduled AlarmManager restart (setExactAndAllowWhileIdle): $reason",
                        )
                    }
                } else {
                    ...
                    android.util.Log.w(
                        "WebSocketService",
                        "SCHEDULE_EXACT_ALARM permission not granted - using inexact alarm",
                    )
                }
                ...
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "Scheduled AlarmManager restart (setExact): $reason",
                    )
                }
            } else {
                ...
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "WebSocketService",
                        "Scheduled AlarmManager restart (set): $reason",
                    )
                }
            }
    ...
    private fun scheduleAutoRestart(reason: String) {
        try {
            ...
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "Scheduled auto-restart via WorkManager: $reason",
                )
            }
        } catch (e: Exception) {
    ...
    private fun handleHeartbeatAlarm() {
        ...
        if (connectionState.isReady() && webSocket != null) {
            ...
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "Heartbeat alarm: service not connected, skipping ping",
                )
            }
    ...
    private fun scheduleHeartbeatAlarm() {
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
                ...
                if (alarmManager.canScheduleExactAlarms()) {
                    ...
                    if (BuildConfig.DEBUG) {
                        android.util.Log.w(
                            "WebSocketService",
                            "SCHEDULE_EXACT_ALARM permission not granted - using inexact heartbeat",
                        )
                    }
                }
            } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "WebSocketService",
                    "Scheduled heartbeat alarm in ${interval + HEARTBEAT_MARGIN_MS}ms",
                )
            }
        } catch (e: Exception) {
