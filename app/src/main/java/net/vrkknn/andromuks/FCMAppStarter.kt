...
object FCMAppStarter {
    ...
    fun ensureAppIsRunningAndConnected(context: Context) {
        try {
            ...
            if (!isWebSocketConnected) {
                Log.w(TAG, "FCM received but WebSocket disconnected - starting app and connecting")
    ...
    private fun initializeAppAndConnect(context: Context) {
        try {
            ...
            // Trigger WebSocket reconnection via the service
            CoroutineScope(Dispatchers.IO).launch {
                try {
                    Log.d(TAG, "Triggering WebSocket reconnection from FCM")
                    
                    // Use the service's reconnection mechanism
                    WebSocketService.triggerReconnectionFromExternal("FCM received - ensuring WebSocket connection")
