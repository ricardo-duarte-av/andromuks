...
6. Updates the in-memory cache so a later `showEnhancedNotification` rebuild keeps the result: `upgradeMessageToImage` for the image **or downloaded voice clip** (same data slot), `upgradeAvatarsInCache(roomId, senderId, senderIcon)` for the sender avatar. The "me" Person and large icon are not cached per-message — they self-correct on the next rebuild via the now-warm disk cache.
...
- **Image / caption / avatar (Phase 2 enriches).** The worker posts the enriched notification; because the tile lags, the enriched content needs to be the last **two** posts to land — see the image note below.
...
**Enriched (image/caption/avatar) re-posts bump `when`.** When Phase 2 *does* change content, the image/caption is added **in place to the same message** Phase 1 already posted (same message timestamp). The widget refreshes on a newer recency marker, not on data added to an existing message, so it would keep the Phase-1 "Sent an image" render and merely *store* the image (which then flashes one-behind on the next message). The worker's enriched re-post therefore sets `when = max(existing when, now)` so the widget treats it as a genuine update and re-renders onto the image. It stays silent (`setOnlyAlertOnce`) so it never re-buzzes.
