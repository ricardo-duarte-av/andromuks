...
**Also gated on zero dropped frames.** The drain sentinel guarantees every message that *reached the dispatcher* is in the queue, but not that every message reached it. A frame the parse pipeline cannot convert is forwarded as `null` and skipped so the ordered dispatcher keeps advancing (deliberately — stalling would kill sync for the rest of the connection), and its rooms are then indistinguishable from rooms the user left. One `sync_complete` carries ~99 rooms, so pruning on that evidence deletes ~99 rooms from RAM, `RoomListCache` *and* `RoomMetadataStore`, deterministically on every cold start.

`WebSocketService.skippedFrameCount()` counts skips per connection (reset where the parse pipeline is built in `connectToWebsocket`). `onInitComplete` prunes only when it is `0`; otherwise it logs `clear_state diff-prune SKIPPED: N frame(s) were unparseable…` and leaves the cached list alone. **Stale rooms are the correct failure mode for an incomplete batch; deleted rooms are not.**

**Postmortem.** This was not hypothetical. `kotlinxJsonElementToOrgJson` typed primitives by sniffing `JsonPrimitive.content` (which strips quotes for strings and bare literals alike) instead of checking `isString`. Kotlin's `toDoubleOrNull` accepts the `Double.valueOf` grammar, so a member whose *previous* displayname was the string `"NaN"` became `Double.NaN` — and `org.json`'s `put()` rejects NaN/Infinity (`JSON.checkDouble`). The exception unwound the whole conversion, so one historical rename by one user in one room made an entire ~350 KB `sync_complete` unparseable, and the prune then deleted the 99 rooms it carried. Symptom: rooms missing from the list, survivors showing stale previews, and a reinstall changing nothing. Fixed at the source (the converter now branches on `isString`, guarded by `WebSocketFrameParsingTest`); this gate is the second line of defence for the next parse failure.
