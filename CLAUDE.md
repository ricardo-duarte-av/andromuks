...
- **`SyncRepository`** — Sits between the WebSocket and AppViewModel; coordinates event flow. The `_events` SharedFlow has `extraBufferCapacity = 1024` and `BufferOverflow.DROP_LATEST` — burst traffic sheds the newest arrivals. `sync_complete` messages bypass `_events` entirely and go to a separate `syncCompleteChannel` (UNLIMITED). Command acks (`response`/`send_complete`/`error`) bypass `_events` too, onto a dedicated `ackEvents` flow, so high-volume `to_device`/`typing` bursts can't evict a request response (see [docs/MESSAGE_SENDING.md](docs/MESSAGE_SENDING.md#layer-2--non-lossy-ack-flow)).
...
- **Cache classes** (12+) — `RoomTimelineCache`, `RoomListCache`, `RoomMemberCache`, `ProfileCache`, `SpaceListCache`, `RoomAccountDataCache`, etc. Singleton in-memory stores shared across Activities. `RoomStateStore` holds per-room Matrix state and is the only one with its own SQLite table (see [docs/ROOM_STATE.md](docs/ROOM_STATE.md)). `IntelligentMediaCache` **loads** media only for `ConversationsApi` (shortcut icons) and `PersonsApi`/`EnhancedNotificationDisplay` (notification avatars); timeline media and UI avatars are loaded by Coil, though they do consult `IntelligentMediaCache.getCachedFile` for an already-cached file. `RoomAccountDataCache` holds per-room account data (e.g. `fi.mau.gomuks.preferences`) populated from sync.
...
| Inline media galleries (MSC4274, shared media parser, mosaic, pager viewer) | [docs/GALLERIES.md](docs/GALLERIES.md) |
| Keyboard GIF / sticker insertion (EditorInfo mime types, InterceptPlatformTextInput, `is_animated`) | [docs/RICH_CONTENT_INPUT.md](docs/RICH_CONTENT_INPUT.md) |
...
| Per-room state cache (`RoomStateStore`, complete-vs-partial responses, persistence, permissions) | [docs/ROOM_STATE.md](docs/ROOM_STATE.md) |
